...
class Solution
{
   public:
    ...
    {
        // union find?
        // can we not just go through it and sort by frequency???

        vector<long long> count(edges.size(), 0);
        ...
        // get max
        long long maxVal = *max_element(count.begin(), count.end());
...
{
    Solution obj;
    vector<int> v1{1, 0, 0, 0, 0, 7, 7, 5};

    dump(obj.edgeScore(v1));
    return 0;
}