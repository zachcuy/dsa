...
output[i] = std::max(output[i] + int, input[i]) == output[i] + int ? true : false

2 3 5 1 3, 3
T T T F T

find the maximum value and subtract the value of extra candies
5 - 3 = 2
then the lowest value that will work is 2
...
public:
    ...
    {
        int minVal = *std::max_element(candies.begin(), candies.end()) - extraCandies;
