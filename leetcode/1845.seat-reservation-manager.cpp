...
use a heap (c++ equivalent is priority_queue)
init with std::greater<int> so we have min heap

reserve(int) -> int:
    return the lowest value in the heap and then pop it
...
{
public:
    SeatManager(int n) // initializer
    {
        // number of seats
        this->n = n;

        // make sure that the heap has all the seats available
        for (int i = 0; i < n; ++i)
        {
            this->heap.push(i + 1);
        }
    ...
    int reserve()
    {
        // get lowest value that is available and take it off the seat manager
        int out = heap.top();
        heap.pop();
