 ...
 */
    ...
    namespace _detail
    {
        ...
        template<typename T>
        bool _dump_one(std::string& output,
                       ...
                       const T& value)
        {
            ...
            auto make_prefix_and_value_str =
                [&, fail_on_newline_in_value](
                    const std::string& prefix,
                    const std::string& indent) -> prefix_and_value_str
            {
                auto last_line_length = get_last_line_length(output + prefix);

                std::string value_str = export_var(value,
                                                   indent,
                                                   last_line_length,
                                                   0,
                                                   fail_on_newline_in_value,
                ...
                bool over_max_line_width =
                    last_line_length + get_first_line_length(value_str)
                    > max_line_width;
            ...
            if (!print_expr)
            {
                prefix_and_value_str pattern1 =
                    make_prefix_and_value_str("", initial_indent);
            ...
            if (fail_on_newline_in_value)
            {
                prefix_and_value_str pattern1a = make_prefix_and_value_str(
                    expr_with_es + es::log(" => "), initial_indent);
                ...
                prefix_and_value_str pattern2a = make_prefix_and_value_str(
                    "\n" + initial_indent + expr_with_es + es::log(" => "),
                    initial_indent);
                   ...
                   const Args&... args)
        {
            ...
            return (...
                    && _dump_one(
                        output, log_label, always_newline_before_expr, *it++, args));
        }
        ...
        template<typename... Args>
        void cpp_dump_macro(_source_location loc,
                            std::initializer_list<std::string_view> exprs,
                            const Args&... args)
        {
            std::string log_label = log_label_func
                ? log_label_func(loc.file_name, loc.line, loc.function_name)
                : "";

            bool exprs_have_newline =
                print_expr && std::any_of(exprs.begin(), exprs.end(), has_newline);
